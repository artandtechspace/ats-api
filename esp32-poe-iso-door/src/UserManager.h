...
public:
    ...
    {
        for (uint16_t i=0; i<EEPROMM_SIZE; i++)
        {
            EEPROM.write(i, 0);
        }
    }
    ...
    static bool StoreNewUser(kUser* pk_NewUser)
    {
        for (int U=0; true; U++)
        {
            ...
            if (Utils::stricmp(pk_NewUser->s8_Name, k_OldUser.s8_Name) < 0)
            {
    ...
    static bool DeleteUser(uint64_t u64_ID, char* s8_Name)
    {
        ...
        for (int U=0; true; U++)
        {
            ...
            if ((u64_ID  != 0    && u64_ID == k_User.ID.u64) ||
                (s8_Name != NULL && Utils::stricmp(s8_Name, k_User.s8_Name) == 0))
            {
    ...
    static bool SetUserFlags(char* s8_Name, byte u8_NewFlags)
    {
        ...
        for (int U=0; true; U++)
        {
            ...
            if (Utils::stricmp(s8_Name, k_User.s8_Name) == 0)
            {
...
private:
    ...
    static bool WriteUserAt(int s32_Index, kUser* pk_User)
    {
        ...
        if (P + sizeof(kUser) > EEPROMM_SIZE)
            return false;
    ...
    {
        kUser k_User;
        int s32_Last = (EEPROMM_SIZE / sizeof(kUser)) -1;
